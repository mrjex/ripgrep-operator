...
runAnalysysTests() {
    ...
    juju run ripgrep-operator/2 analyze-debian architecture=arm64 country=us release=bullseye

    # Get top 5 packages for amd64 from UK mirror in JSON format
    juju run ripgrep-operator/2 analyze-debian architecture=amd64 country=uk release=bookworm count=5 format=json
}
